...
export function RequestDesignEstimateDialog({
  customerType,
  onClose,
  closeDialog,
}: Props) {
  ...
  const form = useFormik({
    ...
    onSubmit: async (values: {
      ...
      try {
        ...
        gaService.event(
          'Request Submitted',
          'Request Design Estimate Form Submitted'
        );
        await leadApiService.requestDesignEstimate({ ...values, customerType });
        alertService
          .notify(
            'Thank You!',
            'Thank you for requesting an estimate for our design services. We will contact you within 24 hours to discuss your project details and provide a quote.',
            'Okay'
          )
          .then(() => {
            closeDialog();
  ...
  return (
    <div className="w-full sm:w-505 max-h-100vh">
      ...
      <h5 className="text-primary text-center text-24 font-medium mt-0 mb-20 mx-30 sm:mx-60">
        Get Started
      </h5>
      <form
        className="w-350 mx-auto mb-40 px-20 pb-60 md:pb-0"
      ...
      >
        ...
        <PhoneInput
          name="phone"
          label="Phone Number"
          placeholder="(300) 448 5543"
          value={form.values.phone}
          onChange={form.handleChange}
        />
        <TextArea
          name="comment"
          label="Please tell us a bit more about your project."
          placeholder=""
          rows={4}
          value={form.values.comment}
          onChange={form.handleChange}
        />
        <p className="text-primary text-18 font-medium text-center my-10">
          Attachments
        </p>
        <p className="text-center mb-10">
          Upload a photo(s) of area or feel free to upload examples you like.
        </p>
        <PhotoUploader
          name="attachments"
          align="center"
